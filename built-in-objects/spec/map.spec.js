describe("Maps",function(){
    ...
    it("should find the correct item when has is called", function(){
        var map = new Map([['name','John'],['age',15],['weight','155']]);
        expect(map.has('age')).toBe(true);//there's an item with that key
        
    })
    it("should not allow duplicate keys", function(){
        var map = new Map();
        var key = {};
        //added two items with the same key
        map.set(key,'first');
        map.set(key,'second');
    ...
    it("should return an iterator of arrays of key value pairs when entries is called", function(){
        ...
        expect(first[0]).toBe('name') //key
        expect(first[1]).toBe('Joe') //value
    })
    it("should return an iterator of values when values is called", function(){
        var map = new Map();
        map.set(1,'a')
        var values = map.values();
        var first = values.next().value; //returns value only
        expect(first).toBe('a');
    })
    it("should return an iterator of keys when keys is called", function(){
        ...
        map.set(1,'a')
        var keys = map.keys();
        var firstKey = keys.next().value; //returns value only
